...
double GetDegree(int i,int j1,int j2)
{
	double r1=atan2(x[j1]-x[i],y[j1]-y[i]),
		r2=atan2(x[j2]-x[i],y[j2]-y[i]),
		r=fabs(r1-r2);
	return min1(r,PI-r)*180/PI;
}
...
int main()
{
	...
	while(cin>>R>>n)
	{
		...
		for(int i=1;i<n;++i)
		{
			...
			{
				w[P(0,0)][P(i,0)]=sqrt(GetDistance2(0,i))+GetDegree(0,n-1,i);
			}
		}
		for(int i=0;i<n;++i)
		{
			for(int j=0;j<n;++j)
			{
				for(int k=0;k<n;++k)
				{
					...
					{
						w[P(j,i)][P(k,j)]=sqrt(GetDistance2(j,k))+GetDegree(j,i,k);
					}
				}
			}
		}
